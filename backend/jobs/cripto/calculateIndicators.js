...
const { calculateEMA } = require('./indicators/calculateEMA');
const { calculateEstocastico } = require('./indicators/calculateEstocastico');
const { calculateMACD } = require('./indicators/calculateMACD');
const { calculateRSI } = require('./indicators/calculateRSI');

function calculateIndicators(dados, cotacaoDolar) {
  const agrupados = agruparDados(dados);

  const rsi = calculateRSI(agrupados);
  const ema = calculateEMA(agrupados, cotacaoDolar);
  const estocastico = calculateEstocastico(agrupados);

  return {
    indicadores: {
      rsi: rsi.rsi,
      ema,
      estocastico: estocastico.estocastico
    }
  }
}
