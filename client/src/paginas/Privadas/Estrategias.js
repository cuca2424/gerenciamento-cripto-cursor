...
import { FaFilter, FaSort, FaSortUp, FaSortDown, FaTrash, FaPlus, FaBell, FaBellSlash, FaEdit, FaChevronLeft, FaChevronRight, FaEye } from 'react-icons/fa';
...
function Estrategias() {
  ...
  const strategiesContainerRef = useRef(null);
  ...
  const handlePeriodChange = (e) => {
    ...
    if (selectedStrategy) {
      fetchCryptos(selectedStrategy.filtros);
    } else {
      fetchCryptos();
    }
  };
  ...
  const handleValueSearch = (searchTerm, item) => {
    ...
    if (/^\d*\.?\d*$/.test(searchTerm)) {
      ...
      handleRemoveStrategyFilter(
        ...
        const existingFilter = strategyFilters.find(f => 
  ...
  const handleCreateStrategy = async () => {
    try {
      ...
      // Verificar se o usuário está tentando criar uma estratégia global
      if (selectedStrategy && selectedStrategy.tipo === 'global' && !user.isAdmin) {
        alert('Apenas administradores podem criar estratégias globais');
        return;
      }

    if (!strategyName.trim()) {
      alert('Por favor, insira um nome para a estratégia');
      return;
    }

      const response = await fetch(`${process.env.REACT_APP_ENDPOINT_API}/estrategia`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          nome: strategyName,
          filtros: strategyFilters,
          tipo: selectedStrategy ? selectedStrategy.tipo : 'pessoal'
        })
      });
  ...
  const handleViewStrategy = async (strategy) => {
    ...
    setStrategyName(strategy.nome);
    setStrategyFilters(strategy.filtros);
    setShowCreateModal(true);
    setEditingStrategyId(strategy._id);
  };

  const handleEditStrategy = async (strategy) => {
    // Verificar se é uma estratégia global e se o usuário tem permissão para editá-la
    if (strategy.tipo === 'global' && !user.isAdmin) {
      alert('Apenas administradores podem editar estratégias globais');
      return;
    }
  ...
  const handleUpdateStrategy = async () => {
    try {
      ...
      // Verificar se é uma estratégia global e se o usuário tem permissão para editá-la
      if (selectedStrategy.tipo === 'global' && !user.isAdmin) {
        alert('Apenas administradores podem editar estratégias globais');
      return;
    }

      const response = await fetch(`${process.env.REACT_APP_ENDPOINT_API}/estrategia/${editingStrategyId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDeleteStrategy = async (strategyId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.REACT_APP_ENDPOINT_API}/estrategia/${strategyId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  // Função para verificar se o usuário está inscrito nas notificações de uma estratégia global
  const isUserSubscribed = (strategy) => {
    if (strategy.tipo === 'global') {
      // Verifica se o array existe e se contém o ID do usuário
      return Array.isArray(strategy.notificacoes) && 
             strategy.notificacoes.some(id => {
               // Se o ID for um ObjectId, converte para string antes de comparar
               const idStr = id._id ? id._id.toString() : id.toString();
               return idStr === user.id;
             });
    }
    return strategy.notificacoes;
  };

  const toggleNotification = async (strategyId) => {
    try {
      const token = localStorage.getItem('token');
      ...
      // Determinar o novo estado baseado no estado atual
      const isCurrentlySubscribed = isUserSubscribed(strategy);
      ...
      // Usar o endpoint específico para notificações
      const response = await fetch(`${process.env.REACT_APP_ENDPOINT_API}/estrategia/${strategyId}/notificacoes`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ notificacoes: newStatus })
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erro ao atualizar notificações: ${JSON.stringify(responseData)}`);
      }
      ...
      // Atualiza a lista de estratégias com o novo valor
  ...
  // Atualizar o estado inicial das notificações ao carregar as estratégias
  useEffect(() => {
    if (strategies.length > 0) {
      const initialNotificationStatus = {};
      strategies.forEach(strategy => {
        initialNotificationStatus[strategy._id] = isUserSubscribed(strategy);
      });
      setNotificationStatus(initialNotificationStatus);
    }
  }, [strategies]);

  const handleAddStrategyFilter = (indicador, tipo, periodo, timeframe, operador, valor) => {
    // Remove o filtro existente se houver
    const newFilters = strategyFilters.filter(f => 
      !(f.indicador === indicador.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') && 
        f.tipo === tipo && 
    ...
    setStrategyFilters(newFilters);
  };

  const handleRemoveStrategyFilter = (indicador, tipo, periodo, timeframe) => {
  ...
  const handleStrategyClick = async (strategy) => {
    // Se a estratégia já estiver selecionada, desseleciona
    ...
    // Seleciona a estratégia e aplica os filtros
    setSelectedStrategy(strategy);
    setStrategyFilters(strategy.filtros || []);
    fetchCryptos(strategy.filtros || []);
  };

  // Função para obter a classe do card da estratégia
  const getStrategyCardClass = (strategy) => {
    const baseClasses = 'card h-100 shadow-sm';
    return selectedStrategy?._id === strategy._id 
  ...
  const fetchEstrategias = async () => {
    try {
      const token = localStorage.getItem('token');
      console.log('Buscando estratégias...');
      console.log('URL:', `${process.env.REACT_APP_ENDPOINT_API}/estrategia`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erro ao carregar estratégias: ${errorData}`);
      }

      const data = await response.json();
      console.log('Estratégias carregadas com sucesso:', data);
      
      // Ordenar estratégias: globais primeiro, depois pessoais
      const estrategiasOrdenadas = data.sort((a, b) => {
        if (a.tipo === 'global' && b.tipo !== 'global') return -1;
        if (a.tipo !== 'global' && b.tipo === 'global') return 1;
        return 0;
      });
      
      setStrategies(estrategiasOrdenadas);
      
      // Inicializar o estado de notificações
      const notificationState = {};
      data.forEach(strategy => {
        notificationState[strategy._id] = strategy.notificacoes || false;
      });
      setNotificationStatus(notificationState);
    } catch (err) {
      console.error('Erro detalhado ao carregar estratégias:', err);
      setError('Erro ao carregar estratégias. Por favor, tente novamente.');
    }
  };

  // Verificar se precisa de navegação
  useEffect(() => {
    const checkOverflow = () => {
  ...
  return (
    <div className="container-fluid px-2 py-2">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        >
          <FaPlus className="me-2" /> Nova Estratégia
        </button>
        </div>
      </div>
      ...
      {strategies.length > 0 && (
        <div className="position-relative mb-3">
          ...
          <div
            ...
            className="d-flex flex-nowrap row gap-0"
            style={{
              overflowX: 'hidden',
          ...
          >
            {strategies.map(strategy => (
              <div key={strategy._id} className='col-md-3' style={{padding: '0px 8px'}}>
                <div
                  className={getStrategyCardClass(strategy)}
                  style={{ cursor: 'pointer', transition: 'border-color 0.3s ease' }}
                  onClick={() => handleStrategyClick(strategy)}
                >
                  <div className="card-header d-flex justify-content-between align-items-center">
                    <h5 className="mb-0">{strategy.nome}</h5>
                    <div className="d-flex align-items-center gap-2" onClick={e => e.stopPropagation()}>
                      <button 
                        className={`btn btn-link text-${isUserSubscribed(strategy) ? 'warning' : 'secondary'} p-0`}
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleNotification(strategy._id);
                        }}
                        title={isUserSubscribed(strategy) ? 'Desativar notificações' : 'Ativar notificações'}
                        style={{ fontSize: '24px' }}
                      >
                        {isUserSubscribed(strategy) ? <FaBell /> : <FaBellSlash />}
                      </button>
                      {strategy.tipo === 'pessoal' && (
                      <button
                  ...
                  <div className="card-body">
                    <p className="text-muted mb-0">
                      {strategy.filtros.length} {strategy.filtros.length === 1 ? 'filtro' : 'filtros'} configurados
                    </p>
              </div>
            </div>
          </div>
      ...
      <div className="row g-3">
        <div className="col-12">
          <div 
            id="cryptoTableCard"
            className="card"
            style={strategies.length > 0 ? cardStyles.comFiltros : cardStyles.semFiltros}
          >
              ...
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">
                    {editingStrategyId ? 
                      (selectedStrategy?.tipo === 'global' ? 'Visualizar Estratégia Global' : 'Editar Estratégia') 
                      : 'Nova Estratégia'}
                  </h5>
                    ...
                    <div className="row mb-3">
                      <div className="col-md-12">
                        ...
                        <input
                          ...
                          disabled={selectedStrategy?.tipo === 'global'}
                        />
                      </div>
                    </div>
                                    ...
                                    <input
                                      ...
                                      placeholder="Valor"
                                        ...
                                        value={inputValues[`${item.indicador.nome}-${item.tipo}-${item.periodo}-${item.timeframe}`] || 
                                               formatIndicatorValue(strategyFilters.find(f => 
                                                 f.indicador === item.indicador.nome.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') && 
                                        f.tipo === item.tipo && 
                                        f.periodo === `periodo${item.periodo}` && 
                                        f.timeframe === item.timeframe
                ...
                <div className="modal-footer">
                  ...
                  <button type="button" className="btn btn-primary" onClick={editingStrategyId ? handleUpdateStrategy : handleCreateStrategy}>
